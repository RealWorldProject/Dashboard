...
empForm.onsubmit = async (e) => {
  ...
  if (btnAdd.innerText == "ADD EMPLOYEE") {
    ...
    addEmployee = await addEmployee.json();
    if (addEmployee.error) {
      M.toast({ html: addEmployee.error, displayLength: 2000 });
      return;
    }
...
const addToEmpTable = (employees) => {
  ...
  for (const employee of employees) {
    ...
    <tr class ="row${employee._id}">
      ...
      <td>
        <button class="editbtn btn"  id="edit-${employee._id}"><i class="material-icons edit">create</i></button>
        <button class="removebtn btn" id="${employee._id}" ><i class="material-icons trash">clear</i></button></td>
        </td>
		</tr>
    `;
    sn++;
