...
devForm.onsubmit = async (d) => {
  ...
  if (btnAdd.innerText == "ADD DEVICE") {
    addDevice = await fetch("http://localhost:5000/admin/device/add-device", {
      method: "POST",
      headers: {
...
const allDevices = async () => {
  let allDevices = await fetch(`http://localhost:5000/admin/device/all-device`);
  ...
  for (const device of devices) {
    html += `
      <tr class ="row${device._id}">
        <td>${sn}</td>
        <td><a href="/billing/device-dashboard/device/${device._id}">${
      device.name
    }</a></td>
        <td>${device.macaddress}</td>
        <td>${device.status ? "Active" : "Not Active"}</td>
        <td>
          <button class="editbtn btn"  id="edit-${
            device._id
          }"><i class="material-icons edit">create</i></button>
          <button class="removebtn btn" id="${
            device._id
          }" ><i class="material-icons trash">clear</i></button></td>
          </td>
          </tr>
      `;
    sn++;
